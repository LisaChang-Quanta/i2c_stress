...
function get_board_type()
{
  ...
  local board_type_tmp=""

  while [ $retry -lt $i2c_retry_time ]; do
    ret=$(pldmtool raw -m 0x0a -d 0x80 0x3f 0x01 0x15 0xa0 0x00 0x18 0x52 0x09 0x4c 0x01 0x1A 2>/dev/null | grep 'Rx:')
    if [[ $? -eq 0 && -n "$ret" ]]; then
      completion_code=$(echo "$ret" | awk '{print $12}')
      board_type_tmp=$(echo "$ret" | awk '{print $13}')
      if [[ $completion_code == 00 ]]; then
        board_type=$board_type_tmp
        case $board_type in
          "00")
  ...
  log_message "${Error}Failed to get board type after $i2c_retry_time attempts.${NoColor}"
  return 1
}
...
local retry=0
  ...
  while [ $retry -lt $i2c_retry_time ]; do
    ret=$(pldmtool raw -m 0x0a -d 0x80 0x3f 0x01 0x15 0xa0 0x00 0x18 0x52 0x09 0x4c 0x01 0x1B 2>/dev/null | grep 'Rx:')
    if [[ $? -eq 0 && -n "$ret" ]]; then
      completion_code=$(echo "$ret" | awk '{print $12}')
      rev_id_tmp=$(echo "$ret" | awk '{print $13}')
      if [[ $completion_code == 00 ]]; then
        rev_id=$rev_id_tmp
        fab_type=$((rev_id + 1))
        case $rev_id in
  ...
  log_message "${Error}Failed to get board rev id after $i2c_retry_time attempts.${NoColor}"
  return 1
}
...
function get_vr_vendor_type()
{
 local retry=0
  local ret=""
  local completion_code=""
  local vr_verndor_type_tmp=""

  while [ $retry -lt $i2c_retry_time ]; do
    ret=$(pldmtool raw -m 0x0a -d 0x80 0x3f 0x01 0x15 0xa0 0x00 0x18 0x52 0x09 0x4c 0x01 0x1C 2>/dev/null | grep 'Rx:')
    if [[ $? -eq 0 && -n "$ret" ]]; then
      completion_code=$(echo "$ret" | awk '{print $12}')
      vr_verndor_type_tmp=$(echo "$ret" | awk '{print $13}')
      if [[ $completion_code == 00 ]]; then
        vr_verndor_type=$vr_verndor_type_tmp
  ...
  log_message "${Error}Failed to get vr vendor type after $i2c_retry_time attempts.${NoColor}"
  return 1
}
...
function get_tmp_type()
{
 local retry=0
  ...
  while [ $retry -lt $i2c_retry_time ]; do
    ret=$(pldmtool raw -m 0x0a -d 0x80 0x3f 0x01 0x15 0xa0 0x00 0x18 0x52 0x01 0x98 0x01 0xFE 2>/dev/null | grep 'Rx:')
    if [[ $? -eq 0 && -n "$ret" ]]; then
      completion_code=$(echo "$ret" | awk '{print $12}')
      if [[ $completion_code == 00 ]]; then
        tmp_type=1  # TMP432
  ...
  tmp_type=2
  log_message "${Warning}Failed to get TMP type after $i2c_retry_time attempts, default to EMC1413.${NoColor}"
  return 1
}
...
function check_power_on()
{
  ...
  while [ $retry -lt $i2c_retry_time ]; do
    ret=$(pldmtool raw -m 0x0a -d 0x80 0x3f 0x01 0x15 0xa0 0x00 0x18 0x52 0x09 0x4c 0x01 0x07 2>/dev/null | grep 'Rx:')
    if [[ $? -eq 0 && -n "$ret" ]]; then
      completion_code=$(echo "$ret" | awk '{print $12}')
  ...
  log_message "${Error}Failed to get power status after $i2c_retry_time attempts.${NoColor}"
  return 1
}
...
main() {
  ...
  check_power_on
  if [ $? -ne 0 ]; then
    log_message "${Error}check_power_on failed.${NoColor}"
    return 1
  fi
  get_board_type
  if [ $? -ne 0 ]; then
    log_message "${Error}get_board_type failed.${NoColor}"
    return 1
  fi
  get_board_rev_id
  if [ $? -ne 0 ]; then
    log_message "${Error}get_board_rev_id failed.${NoColor}"
    return 1
  fi
  get_vr_vendor_type
  if [ $? -ne 0 ]; then
  ...
  if [[ $stress_fail_flag -ne 0 ]]; then
    log_message "${Error}One or more stress tests failed.${NoColor}"
    return 1
  fi
...
if [ $main_rc -ne 0 ]; then
  log_message "${Error} Script failed with code ${main_rc}.${NoColor}"
else
  log_message "${Info} Script finished successfully.${NoColor}"
fi
